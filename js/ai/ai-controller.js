...
export async function executeAiTurn(player) {
    ...
    try {
        ...
        if (effectCards.length > 0) {
            ...
            // Specific AI logic for King Reversum
            if (player.aiType === 'reversum') {
                const reversusTotalCard = effectCards.find(c => c.name === 'Reversus Total');
                if (reversusTotalCard && !gameState.reversusTotalActive) {
                    const opponentsWithPositiveEffects = opponents.filter(opp => opp.effects.score === 'Mais' || opp.effects.movement === 'Sobe').length;
                    // Use if at least two opponents have positive effects, to make it feel impactful
                    if (opponentsWithPositiveEffects >= 1) {
                         bestMove = { card: reversusTotalCard, target: player.id, score: 100, reason: "para aniquilar suas esperanças" };
                    }
                }
            }

            // General AI logic if no special move was chosen
            if (bestMove.score < 0) {
                for (const card of effectCards) {
                    ...
                    // Check for 'Reversus Total' (General AI, not Reversum)
