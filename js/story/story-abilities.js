        ...
        if (space && !space.isUsed) {
            ...
            switch (space.color) {
                case 'black': {
                    ...
                    space.isUsed = true;
                     // Special logic for the final battle - now handled by heart loss, so this is for other players
                    if (gameState.currentStoryBattle === 'necroverso_final' && player.aiType === 'necroverso_final') {
                         // Necroverso team members don't get eliminated by their own black holes
                         updateLog(`${player.name} pousou em um buraco negro, mas é imune.`);
                    } else {
